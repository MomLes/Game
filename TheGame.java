...
public class TheGame extends GameThread{
    ...
    public void setupBeginning() {
        ...
        // speed should increase with number of sad balls
        mBallSpeedX = mCanvasWidth / 3 * (1+mNum/3);
        mBallSpeedY = mCanvasHeight / 3 * (1+mNum/3);
        ...
        //Calculate random positions for SadBalls
        // mNum is initially 1; later calculated depending on the score of the last game

        for (int i = 0; i < mNum; i++) {
            mSadBallX[i] = mCanvasWidth * (float) Math.random();
            mSadBallY[i] = mCanvasWidth * (float) Math.random();
        }
    ...
    protected void updateGame(float secondsElapsed) {
        ...
        if(mBallY >= mCanvasHeight) {
            ...
            // Calculate the number of sad balls for the next turn
            double num = score/3.0;
            mNum = (int) num  + 1;

            //the SadBall array is a max of 5
            if (mNum > 5) {
                mNum = 5;
            }
        }
    }
    private void updateBallCollision(float x, float y) {
